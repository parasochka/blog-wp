...
<header class="now-header" style="position:sticky; top:0; z-index:100; background:color-mix(in srgb, var(--bg-page) 78%, transparent); backdrop-filter:saturate(150%) blur(14px); -webkit-backdrop-filter:saturate(150%) blur(14px); border-bottom:1px solid var(--border)">
	<div style="width:100%; max-width:1200px; margin-inline:auto; padding-inline:24px; display:flex; align-items:center; justify-content:space-between; gap:32px; height:68px">
		...
		<div style="display:flex; align-items:center; gap:16px">
			<form role="search" method="get" class="now-search" action="<?php echo esc_url( home_url( '/' ) ); ?>" style="display:inline-flex; align-items:center; gap:8px; height:44px; padding:0 14px; border-radius:999px; border:1px solid var(--border-strong); background:var(--surface-panel); color:var(--text-secondary); font-family:var(--font-ui); font-size:14px">
			...
			?>
			<a href="<?php echo esc_url( $now_cta_url ); ?>"<?php echo $now_cta_rel ? ' target="_blank" rel="' . esc_attr( $now_cta_rel ) . '"' : ''; ?> class="now-cta-accent" style="display:inline-flex; align-items:center; justify-content:center; gap:8px; font-family:var(--font-ui); font-weight:700; font-size:14px; line-height:1; letter-spacing:0.1px; height:44px; padding:0 16px; border-radius:var(--radius-cta); background:var(--accent-400); color:var(--neutral-950); box-shadow:var(--glow-accent); white-space:nowrap"><?php echo esc_html( now_mod( 'now_cta_label' ) ); ?></a>
	...
	<div id="now-mobile-menu" class="now-mobile-menu" hidden>
		<div style="max-width:1200px; margin-inline:auto; padding:6px 24px 16px">
